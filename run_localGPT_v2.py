...
def main(device_type, show_sources, use_history, model_type, save_qa):
    ...
    # Get the directory path of "question_bank"
    current_directory = current_directory = os.getcwd()
    questions_directory = os.path.join(current_directory, "question_bank")

    # Return questions in form of an array and the qn number of the most recent query from that file
    query_array_and_number = utils.run_question_file(questions_directory)
    print(len(query_array_and_number[0]), query_array_and_number[1])


    # Automatic question and answers
    question_counter = 0
    try:
        for input_query in query_array_and_number[0][int(query_array_and_number[1]):]:
            query = input_query
        ...
        #Once it has reached the end of the question file, set question count to 0 
        file_path = os.path.join(questions_directory, "question_number.csv")  # Construct the full file path
        with open(file_path, 'w') as f:
            # write count number into csv file
            writer = csv.writer(f).writerow([0])

    #If user ends automatic querying, save the most recent qn number in a csv file
    except KeyboardInterrupt:
        file_path = os.path.join(questions_directory, "question_number.csv")  # Construct the full file path
        with open(file_path, 'w') as f:
            # write count number into csv file
            writer = csv.writer(f).writerow([question_counter])
